...
GOFLAGS=-a -tags safe-and-sound -installsuffix safe-and-sound -mod=vendor

run:
	go run $(BUILD_PATH)/main.go

bin:
	echo "  >  Building binary \"safe-and-sound\" $(VERSION)..."
	go build $(GOFLAGS) $(LDFLAGS) -o $(BINARY) $(BUILD_PATH)

bin-linux:
	echo "  >  Building linux-amd64 binary \"safe-and-sound\" $(VERSION)..."
	GOOS=linux GOARCH=amd64 go build $(GOFLAGS) $(LDFLAGS) -o $(BINARY)-linux $(BUILD_PATH)

bin-windows:
	echo "  >  Building windows-amd64 binary \"safe-and-sound\" $(VERSION)..."
...
rebuild:
	docker-compose build safe-and-sound
