...
COPY . /go/src/github.com/ra9dev/PROJECTNAME
WORKDIR /go/src/github.com/ra9dev/PROJECTNAME
RUN \
    cd cmd/apiserver && \
    go build -a -tags PROJECTNAME -installsuffix PROJECTNAME -mod=vendor -ldflags "-X main.version=${version} -s -w" -o /go/bin/PROJECTNAME
